...
def read_human_annotations(human_annotations_filename):
	...
	for line in human_annotations_file.readlines():
		...
		if 1 in annotations[post_id]: annotations[post_id].remove(1)
		if 2 in annotations[post_id]: annotations[post_id].remove(2)
		if 3 in annotations[post_id]: annotations[post_id].remove(3)
		if len(annotations[post_id]) == 0:
			print post_id
			annotations[post_id].append(1)
		#annotations[post_id] = best_union
		#annotations[post_id] = [0]
	return annotations

def create_refs(test_ids, annotations, question_candidates, ref_prefix):
...
def main(args):
	...
	create_refs(test_ids, annotations, question_candidates, args.ref_prefix)	
